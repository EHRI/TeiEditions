...
*/
class TeiEditions_BatchEnhancer
{
    ...
    private function enhanceZip($path, TeiEditions_DataFetcher $src, &$added): string
    {
        $temp = tempnam(sys_get_temp_dir(), '');
        $out = new ZipArchive;
        if ($out->open($temp) !== false) {
            ...
            if ($zip->open($path) !== false) {
                for ($i = 0; $i < $zip->count(); $i++) {
                    ...
                    $doc = TeiEditions_DocumentProxy::fromString($xml_str);
    ...
    private function enhanceFile(TeiEditions_DocumentProxy $doc, TeiEditions_DataFetcher $src, &$added): string
    {
        $tool = new TeiEditions_TeiEnhancer($doc, $src);
